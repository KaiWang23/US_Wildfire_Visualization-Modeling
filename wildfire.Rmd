...
fires = fires %>% 
  select(FIRE_YEAR, DISCOVERY_DATE, DISCOVERY_DOY, DISCOVERY_TIME, STAT_CAUSE_CODE, STAT_CAUSE_DESCR, CONT_DATE, CONT_DOY, CONT_TIME, FIRE_SIZE, FIRE_SIZE_CLASS, LATITUDE, LONGITUDE, OWNER_CODE, OWNER_DESCR, STATE, COUNTY, FIPS_CODE, FIPS_NAME, region, CONT_DATEymd, DISCOVERY_DATEymd)
...
shinyApp(
  ...
  server = function(input, output, session){
    ...
    output$animated_image <- renderImage({
      outfile <- tempfile(fileext='.gif')
      saveGIF(plotfire(isolate(input$State), isolate(input$year), isolate(input$month)), movie.name = "outfile.gif")
      contentType = 'image/gif'
      list(src="outfile.gif")
    }, deleteFile = T)})    
