...
1. Here we write a plotfire function to plot gif. We use savegif function to impletement the plot function. With the intial plotdata, we want to show the animation of monthly wildfires of a spcified state, year, and month. It needs a for-loop function to plot daily plots and saveGIFfunction to generate GIF. So it needs quite much time.

2. Then we setup a randomforest model to predict the fire cause. The input is state, month, and day. Our predictors are longitude, latitude, state, fire sizes. This model will help fire policemen to predict the furture causes of wildfires when the policemen are noticed of a fire.
...
#function to get country name by lat and long
#
```

Part(3):Define the interface  of the shiny app and realize their functions:

We use navbarPage and leaflet to build a neat shiny. We have three total tabs, then we place each function in each tab. The first tab is showing the total map of fires. When we put the mouth icon on one point, it will turn out the discover date, unique id, fire size, fire class and cause. On the left pannel, it will show the corresponding fire causes and fire sizes by date.

On the second tab, we also have leaflet map. And this time we want to use our model to predict the cause of fires. When we click on prediction buttons, it will predic the fire causes of points on the map. Then we can enter the longtitude, latitude, firesize and then the model will return a most possible fire cause. It helps firepolice to identify the fire causes when they receive a unclear fire emergency.
  ...
  ui = navbarPage("US Wildfires", id="nav",
    ...
    div(class="outer",
      ...
      absolutePanel(id = "predict_new", class = "panel panel-default", fixed = TRUE,
          ...
          br(),
          numericInput("year.new","Year:",min = 2016,max = 2050,value = 2016),
          br(),
