...
shinyApp(
    ...
    tabPanel("Interactive map",
      div(class="outer",
        ...
        absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
          ...
          dateInput("datefrom", "Select the Start Date", value = "2015-12-25", min = "1992-01-01", max = "2015-12-31"),
  ...
  server = function(input, output, session){
    ...
    observe({
      ...
      fires.df = fires %>% filter((input$datefrom <= DISCOVERY_DATE)&(DISCOVERY_DATE <= input$dateto))
      if (input$state != "All"){
        fires.df = fires.df %>% filter(STATE == input$state)
      }
      
      locations = fires.df %>% select(LONGITUDE, LATITUDE)
      
      #popups = fires.df %>% paste(paste0("<b>", FOD_ID,"</b>"),
                                  #paste0("Discovery Date: ", DISCOVERY_DATE),
