...
getTree=function(state.name,year,month,day_of_week,open=T,new=F){
  ...
  ui = navbarPage("US Wildfires", id="nav",
    ...
    div(class="outer",
      ...
      absolutePanel(id = "predict_new", class = "panel panel-default", fixed = TRUE,
          ...
          br(),
          numericInput("lat.new","Latitude:",min = 0,max = 90,value = 0),
          br(),
          numericInput("long.new","Longitude:",min=-180,max = 180,value = 0),
          br(),
          numericInput("fire.size.new","Fire Size:",min = 0,max = 10,value = 0),
          br(),
          actionButton("go.new", "Predict New Fire!"),
    ...
    observeEvent(input$go.new,
                 {
                   output$cause.new=renderText(
                     as.character(getNew(isolate(input$state.new),isolate(as.numeric(input$year.new)),isolate(as.numeric(input$month.new)),isolate(as.numeric(input$d.of.w.new)),isolate(as.numeric(input$lat.new)),isolate(as.numeric(input$long.new)),isolate(as.numeric(input$fire.size.new)))
